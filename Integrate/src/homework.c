...
double integrate(double x[3], double y[3], double (*f) (double, double))
{
    ...
    xLoc[0] = x[0] * twoThirds + (x[1] + x[2]) * sixth; //  = X0 * 2/3 + X1 * 1/6 + X2 * 1/6 
    xLoc[1] = (x[0] + x[1]) * sixth + x[2] * twoThirds; //  = X0 * 1/6 + X1 * 1/6 + X2 * 2/3
    xLoc[2] = (x[0] + x[2]) * sixth + x[1] * twoThirds; //  = X0 * 1/6 + X1 * 2/3 + X2 * 1/6
    yLoc[0] = y[0] * twoThirds + (y[1] + y[2]) * sixth; //  = Y0 * 2/3 + Y1 * 1/6 + Y2 * 1/6
    yLoc[1] = (y[0] + y[1]) * sixth + y[2] * twoThirds; //  = Y0 * 1/6 + Y1 * 1/6 + Y2 * 2/3
    yLoc[2] = (y[0] + y[2]) * sixth + y[1] * twoThirds; //  = Y0 * 1/6 + Y1 * 2/3 + Y2 * 1/6

    for(unsigned short int i = 0; i < 3; ++i)
        I += f(xLoc[i],yLoc[i]);

    I *= sixth * jacobien(x,y); //poids tous égaux à 1/6
